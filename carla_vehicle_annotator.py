...
def auto_annotate_lidar(vehicles, camera, lidar_data, max_dist = 100, min_detect = 5, show_img = None, json_path = None):
    ...
    if json_path is not None:
        filtered_out['class'] = get_actor_class(visible_vehicles, json_path)
    return filtered_out, filtered_data

### Use this function to get 2D bounding boxes of visible vehicle to camera
def auto_annotate(actors, camera, depth_img, segm_img, depth_location, max_dist=100, depth_margin=-1, patch_ratio=0.5, resize_ratio=0.5, json_path=None):
    depth_show = False
    actors = filter_angle_distance(actors, camera, max_dist)

    camera_location = camera.get_transform().location

    # actor ids
    actor_ids = [ac.id for ac in actors]

    # Calculate the distance between the depth camera and each actor
    actor_distance = [math.sqrt((ac.get_transform().location.x - camera_location.x)**2 + \
                                (ac.get_transform().location.y - camera_location.y)**2 + \
                                (ac.get_transform().location.z - camera_location.z)**2) \
                                 for ac in actors]    
    
    # actor_distance = [math.sqrt((ac.x)) for ac in actors]

    bounding_boxes_2d = [get_2d_bb(actor, camera) for actor in actors]

    if json_path is not None:
        actor_class = get_actor_class(actors, json_path)
    else:
        actor_class = []
    filtered_out, removed_out, _, _ = filter_occlusion_bbox(bounding_boxes_2d, actor_ids, actor_distance, actors, camera, depth_img, actor_class, depth_show, depth_margin, patch_ratio, resize_ratio)

    t_bbs, t_cl = traffic_create_bbox(segm_img)
    t_distance = create_distance(t_bbs, t_cl, segm_img, depth_img)

    filtered_out['bbox'] += t_bbs
    filtered_out['class'] += t_cl
    filtered_out['distance'] += t_distance
    filtered_out['ids'] += ['None'] * len(t_bbs)

    return filtered_out, removed_out 

### Same with auto_annotate(), but with debugging function for the occlusion filter
def auto_annotate_debug(actors, camera, depth_img, depth_show=False, max_dist=100, depth_margin=-1, patch_ratio=0.5, resize_ratio=0.5, json_path=None):
    actors = filter_angle_distance(actors, camera, max_dist)
    bounding_boxes_2d = [get_2d_bb(actor, camera) for actor in actors]
    ...
    if json_path is not None:
        ...
        actor_class = []
    filtered_out, removed_out, depth_area, depth_show = filter_occlusion_bbox(bounding_boxes_2d, actors, camera, depth_img, actor_class, depth_show, depth_margin, patch_ratio, resize_ratio)
    return filtered_out, removed_out, depth_area, depth_show


### Calculate the distance from the depth image
def create_distance(bbox, cl, segm_img, depth_img):

    segm_img.convert(carla.ColorConverter.Raw)
    img_bgra = np.array(segm_img.raw_data).reshape((segm_img.height,segm_img.width,4))
    img_seg = np.zeros((segm_img.height,segm_img.width))
    img_seg = img_bgra[:,:,2]

    distance = []
    for bb, c in zip(bbox, cl):
        seg_roi = img_seg[bb[0][1] : bb[1][1], bb[0][0] : bb[1][0]]
        depth_roi = depth_img[bb[0][1] : bb[1][1], bb[0][0] : bb[1][0]]

        seg_label = 18 if c == 5 else 12
        seg_index = np.where(seg_roi == seg_label)

        distance.append(np.min(depth_roi[seg_index]))

    return distance

### Creating 2D bounding boxes for traffic lights and signs using semantic segmentation images
def traffic_create_bbox(segm_img):
    ...
    ## traffic light
    img_traffic_light = np.where(img_seg == 18, 255, 0)
    img_traffic_light = np.uint8(img_traffic_light)

    image, contours, hierarchy = cv2.findContours(img_traffic_light, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    # bbox size threshold
    contours = list(filter(lambda x: cv2.contourArea(x) > 40, contours))

    for i, cnt in enumerate(contours):

        x1, y1, w, h = cv2.boundingRect(cnt)

        # x2 = x1 + np.maximum(0., w - 1.)
        # y2 = y1 + np.maximum(0., h - 1.)
        x2 = x1 + w
        y2 = y1 + h

        bbs.append(np.array([[x1, y1] , [x2, y2]]))
        cl.append(5)

    
    ## traffic sign
    img_traffic_sign = np.where(img_seg == 12, 255, 0)
    img_traffic_sign = np.uint8(img_traffic_sign)

    image, contours, hierarchy = cv2.findContours(img_traffic_sign, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
...
def filter_occlusion_bbox(bounding_boxes, ids, distance, vehicles, sensor, depth_img, v_class=None, depth_capture=False, depth_margin=-1, patch_ratio=0.5, resize_ratio=0.5):
    filtered_bboxes = []
    filtered_vehicles = []
    filtered_v_class = []
    filtered_ids = []
    ...
    for bbox, ac_id, ac_d, v, s in zip(bounding_boxes, ids, distance, vehicles, selector):
        if s:
            filtered_bboxes.append(bbox)
            filtered_vehicles.append(v)
            filtered_ids.append(ac_id)
            filtered_distance.append(ac_d)

        else:
            removed_bboxes.append(bbox)
            removed_vehicles.append(v)
            removed_ids.append(ac_id)
            removed_distance.append(ac_d)

    filtered_out['bbox']=filtered_bboxes
    filtered_out['vehicles']=filtered_vehicles
    filtered_out['ids']=filtered_ids
    filtered_out['distance']=filtered_distance

    removed_out['bbox']=removed_bboxes
    removed_out['vehicles']=removed_vehicles
    removed_out['ids']=removed_ids
    removed_out['distance']=removed_distance
...
def get_actor_class(vehicles, json_path=None):
    f = open(json_path)
...
def save_output(carla_img, filtered, removed, cc_rgb=carla.ColorConverter.Raw, path='', save_patched=False, add_data=None, out_format='pickle'):

    carla_img.save_to_disk(path + 'out_rgb/%06d.png' % carla_img.frame, cc_rgb)

    out_dict = {}

    bboxes_list = [bbox.tolist() for bbox in filtered['bbox']]
    out_dict['bboxes'] = bboxes_list
    out_dict['class'] = filtered['class']
    out_dict['ids'] = filtered['ids']
    out_dict['distance'] = filtered['distance']

    bboxes_list = [bbox.tolist() for bbox in removed['bbox']]
    out_dict['removed_bboxes'] = bboxes_list
    out_dict['removed_class'] = removed['class']
    out_dict['removed_ids'] = removed['ids']
    out_dict['removed_distance'] = removed['distance']
    ...
    if save_patched:
        ...
        for crop, cl in zip(filtered['bbox'], filtered['class']):
            u1 = int(crop[0,0])
            v1 = int(crop[0,1])
            u2 = int(crop[1,0])
            v2 = int(crop[1,1])
            ...
            img_draw.rectangle(crop_bbox, outline=tuple(LABEL_COLORS[cl]), width=2)
